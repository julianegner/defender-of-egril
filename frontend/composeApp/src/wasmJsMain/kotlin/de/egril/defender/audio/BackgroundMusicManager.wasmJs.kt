...
external fun setMusicAudioLoop(
    audio: JsAny,
    loop: Boolean,
)

@JsFun("(audio, volume) => audio.volume = volume")
external fun setMusicAudioVolume(
    audio: JsAny,
    volume: Double,
)
...
external fun setMusicAudioCurrentTime(
    audio: JsAny,
    time: Double,
)
    ...
    override fun setVolume(volume: Float) {
        ...
        audioElement?.let { audio ->
            try {
                ...
                val effectiveVolume =
                    (AppSettings.soundVolume.value.toDouble() * this.volume.toDouble() * trackVolume.toDouble() * 0.3)
                        .coerceIn(
                            0.0,
                            1.0,
                        )
 ...
 */
actual fun createBackgroundMusicManager(): BackgroundMusicManager = WasmBackgroundMusicManager()
