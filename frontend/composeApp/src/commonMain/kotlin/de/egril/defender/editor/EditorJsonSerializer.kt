...
object EditorJsonSerializer {
    ...
    fun serializeMap(map: EditorMap): String {
        val tilesJson =
            map.tiles.entries.joinToString(",\n    ") { (pos, type) ->
                "\"$pos\": \"${type.name}\""
            }
        ...
        val worldMapPositionJson =
            if (map.worldMapPosition != null) {
        ...
        val riverTilesJson =
            if (map.riverTiles.isNotEmpty()) {
                val riverData =
                    map.riverTiles.entries.joinToString(",\n    ") { (pos, river) ->
        ...
        val targetInfoJson =
            if (map.targetInfoMap.isNotEmpty()) {
                val targetData =
                    map.targetInfoMap.entries.joinToString(",\n    ") { (pos, info) ->
  ...
  "data": $data
                                ...
                                if (flowDirectionMatch != null && flowSpeedMatch != null) {
                                    val flowDirection =
                                        de.egril.defender.model.RiverFlow
                                            .valueOf(flowDirectionMatch.groupValues[1])
    ...
    fun serializeLevel(level: EditorLevel): String {
        val spawnsJson =
            level.enemySpawns.joinToString(",\n    ") { spawn ->
                val spawnPointJson =
        ...
        val subtitleKeyJson =
            if (level.subtitleKey != null) {
                ",\n  \"subtitleKey\": \"${level.subtitleKey}\""
            } else {
                ""
            }

        val requiredCountJson =
            if (level.requiredPrerequisiteCount != null) {
        ...
        val communityDescriptionJson =
            if (level.communityDescription.isNotEmpty()) {
                val escaped =
                    level.communityDescription
                        .replace("\\", "\\\\")
                        .replace("\"", "\\\"")
                        .replace("\n", "\\n")
                        .replace("\r", "\\r")
      ...
      $cooldownData
        ...
        val initialDataJson =
            if (initialData.defenders.isNotEmpty() ||
                ...
                if (initialData.defenders.isNotEmpty()) {
                    val defendersData =
                        initialData.defenders.joinToString(",\n      ") { defender ->
                            val dragonNameJson =
      ...
      $defendersData
                ...
                if (initialData.attackers.isNotEmpty()) {
                    val attackersData =
                        initialData.attackers.joinToString(",\n      ") { attacker ->
                            val healthJson =
                                if (attacker.currentHealth != null) {
      ...
      $attackersData
                ...
                if (initialData.traps.isNotEmpty()) {
                    val trapsData =
                        initialData.traps.joinToString(",\n      ") { trap ->
  ...
  "data": $data
    ...
    fun deserializeLevel(json: String): EditorLevel? {
        ...
        try {
            ...
            for (entry in spawnEntries) {
                ...
                val attackerType =
                    try {
                        ...
                        println(
                            "Warning: skipping enemy spawn with unknown attackerType '$attackerTypeStr' — this type may have been removed from the game.",
                        )
                        continue
                    }
            ...
            for (entry in towerEntries) {
                if (entry.isNotBlank()) {
                    try {
                        ...
                        println(
                            "Warning: skipping unknown tower type '$entry' in availableTowers — this type may have been removed from the game.",
                        )
                    }
                }
            }
            ...
            val testingOnly =
                if (dataJson.contains("\"testingOnly\"")) {
                    try {
                    ...
                    if (initialDataSection.contains("\"defenders\"")) {
                        ...
                        if (defendersSection.isNotBlank()) {
                            ...
                            for (entry in defenderEntries) {
                                ...
                                if (LogConfig.ENABLE_LEVEL_LOADING_LOGGING) {
                                    println(
                                        "EditorJsonSerializer: Added InitialDefender(type=$type, position=$position, level=$level, dragonName=$dragonName, onTowerBase=$onTowerBase)",
                                    )
                                }
                            }
                            if (LogConfig.ENABLE_LEVEL_LOADING_LOGGING) {
  ...
  "data": {
    ...
    fun serializeWorldMapData(data: WorldMapData): String {
        val locationsJson =
            data.locations.joinToString(",\n    ") { location ->
                val levelIdsJson = location.levelIds.joinToString(", ") { "\"$it\"" }
                val nameKeyJson =
                    if (location.nameKey != null) {
                        """,
      "nameKey": "${location.nameKey}""""
                    } else {
                        ""
                    }
                val iconResourceNameJson =
                    if (location.iconResourceName != null) {
                        """,
      ...
      "levelIds": [$levelIdsJson]$iconResourceNameJson
        ...
        val pathsJson =
            data.paths.joinToString(",\n    ") { path ->
                val controlPointsJson =
