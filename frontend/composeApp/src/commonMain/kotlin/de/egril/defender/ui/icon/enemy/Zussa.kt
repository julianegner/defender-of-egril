...
/**
 * Draw Zussa symbol (disabling twin, villain).
 * Based on the red witch but with a distinctive golden star on the hat tip to mark her
 * as a unique villain rather than an ordinary red witch.
 */
fun DrawScope.drawZussaSymbol(
    centerX: Float,
    centerY: Float,
    size: Float,
    ...
    withTransform({ scale(headScale, headScale, Offset(centerX, headCenterY)) }) {
        ...
        // Deep crimson hat (darker than regular red witch to distinguish)
        val hatPath =
            Path().apply {
                moveTo(centerX, centerY - size * 0.38f)
                lineTo(centerX - size * 0.26f, centerY - size * 0.05f)
        ...
        drawPath(hatPath, Color(0xFF6B0000)) // Deep crimson
        ...
        // Face: slightly warmer than regular red witch
        drawCircle(
            color = Color(0xFFFFC0C0), // Slightly rosy face
            radius = size * 0.18f,
            center = Offset(centerX, headCenterY),
        )
    ...
    // Wand with crackling disable orb (not scaled)
    drawLine(
