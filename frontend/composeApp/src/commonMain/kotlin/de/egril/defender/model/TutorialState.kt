...
data class TutorialState(
     ...
     */
    fun shouldShowOverlay(): Boolean = isActive && currentStep != TutorialStep.NONE
    ...
    fun getNextStep(): TutorialStep =
        when (currentStep) {
            ...
            TutorialStep.ATTACKING -> {
                if (hasAttackedEnemy || canSkipAttacking) {
     ...
     */
    fun markTowerPlaced(): TutorialState = copy(hasPlacedFirstTower = true)

    /**
     * Mark that the player has started their first turn
     */
    fun markTurnStarted(): TutorialState = copy(hasStartedFirstTurn = true)

    /**
     * Mark that the player has attacked an enemy
     */
    fun markAttackedEnemy(): TutorialState = copy(hasAttackedEnemy = true)

    /**
     * Allow skipping the attacking step (tower can't reach or action used)
     */
    fun allowSkipAttacking(): TutorialState = copy(canSkipAttacking = true)
