...
object LevelData {
    fun createLevels(): List<Level> {
        ...
        val validLevels =
            sequence.sequence.mapIndexedNotNull { index, levelId ->
                ...
                editorLevel?.let { level ->
                    ...
                    if (!EditorStorage.isLevelReadyToPlay(level)) {
                        println(
                            "Skipping level $levelId: not ready to play (towers: ${level.availableTowers.size}, spawns: ${level.enemySpawns.size}, map ready: ${map?.readyToUse})",
                        )
