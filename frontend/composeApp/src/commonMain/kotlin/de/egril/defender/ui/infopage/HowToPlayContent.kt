...
@Composable
private fun HowToPlayBulletPointWithIcon(
            ...
            val description =
                when {
                    ...
                    defenderType.baseDamage == 0 -> stringResource(Res.string.special_building)
                        ...
                        "${defenderType.baseCost} $coinsLabel | ${defenderType.baseDamage} ${stringResource(
                            Res.string.damage,
                        )} | $rangeLabel ${defenderType.baseRange} | ${defenderType.attackType.getLocalizedName(locale)}"
                    }
                }
...
private fun HowToPlayEnemyInfo(attackerType: AttackerType) {
    ...
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            ...
            Text(
                text = "${attackerType.health} ${stringResource(
                    Res.string.hp_label,
                )} | ${stringResource(
                    Res.string.speed_label,
                )} ${attackerType.speed} | ${attackerType.reward} ${stringResource(Res.string.coins_label)}",
                style = MaterialTheme.typography.bodySmall,
                fontSize = 12.sp,
            )
        }
