...
class MineOperations(
    ...
    private fun spawnDragonFromMine(mine: Defender) {
        ...
        val closestTarget =
            state.level.targetPositions.minByOrNull {
        ...
        val spawnPosition =
            if (unitAtPosition != null && unitAtPosition.type == AttackerType.EWHAD) {
                ...
                if (unitAtPosition != null) {
                    println(
                        "Dragon eating ${unitAtPosition.type} on spawn at $closestPathPos, gaining ${unitAtPosition.currentHealth.value} HP",
                    )
