...
fun SavedGameCard(
    ...
    val level =
        remember(saveGame.levelId, saveGame.mapId) {
    ...
    val minimapGameState =
        remember(saveGame.id) {
            if (level != null &&
                (
                    saveGame.defenderPositions.isNotEmpty() ||
                        saveGame.attackerPositions.isNotEmpty() ||
                        saveGame.barricadePositions.isNotEmpty()
                )
            ) {
                // Create minimal Defender/Attacker objects for minimap display
                val defenders =
                    saveGame.defenderPositions.map { saved ->
                        Defender(
                            id = saved.id,
                            type = saved.type,
                ...
                val attackers =
                    saveGame.attackerPositions.filter { !it.isDefeated }.map { saved ->
                        Attacker(
                            id = saved.id,
                            type = saved.type,
                            position = mutableStateOf(saved.position),
                ...
                val barricades =
                    saveGame.barricadePositions.map { saved ->
                        Barricade(
                            id = saved.id,
                ...
                GameState(
                    ...
                    defenders =
                        androidx.compose.runtime.snapshots
                            .SnapshotStateList<Defender>()
                            .apply { addAll(defenders) },
                    attackers =
                        androidx.compose.runtime.snapshots
                            .SnapshotStateList<Attacker>()
                            .apply { addAll(attackers) },
                    barricades =
                        androidx.compose.runtime.snapshots
                            .SnapshotStateList<Barricade>()
                            .apply { addAll(barricades) },
                )
            } else {
                null
            }
