...
object PlayerProfileStorage {
     ...
     * Get a profile by ID
     */
    fun getProfile(playerId: String): PlayerProfile? = getAllProfiles().profiles.find { it.id == playerId }
    ...
    fun deleteProfile(playerId: String): Boolean {
        ...
        val updatedProfiles =
            profiles.copy(
                ...
                lastUsedPlayerId =
                    if (profiles.lastUsedPlayerId == playerId) {
     ...
     */
    fun getPlayerDirectory(playerId: String): String = "$PLAYERS_DIR/$playerId"

    /**
     * Get the directory path for a player's save files
     */
    fun getPlayerSavefilesDirectory(playerId: String): String = "$PLAYERS_DIR/$playerId/savefiles"

    /**
     * Get the file path for a player's level progress
     */
    fun getPlayerLevelProgressFile(playerId: String): String = "$PLAYERS_DIR/$playerId/level_progress.json"
     ...
     */
    fun findByRemoteUsername(remoteUsername: String): PlayerProfile? = getAllProfiles().profiles.find { it.remoteUsername == remoteUsername }
    ...
    fun addAchievement(
        playerId: String,
        achievement: de.egril.defender.model.Achievement,
    ): Boolean {
        val profiles = getAllProfiles()
        val profile = profiles.profiles.find { it.id == playerId } ?: return false
    ...
    fun hasAchievement(
        playerId: String,
        achievementId: de.egril.defender.model.AchievementId,
    ): Boolean {
    ...
    fun migrateExistingSaves(): PlayerProfile? {
        ...
        val hasExistingSaves =
            fileStorage.fileExists("savefiles/level_progress.json") ||
