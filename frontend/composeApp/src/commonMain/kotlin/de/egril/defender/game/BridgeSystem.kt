...
class BridgeSystem(
     ...
     */
    private fun isBridgeStrategic(
        attacker: Attacker,
        bridgePositions: List<Position>,
    ): Boolean {
        val pathfinding =
            de.egril.defender.game
                .PathfindingSystem(state)
     ...
     */
    private fun selectBestRiverForWizard(
        attacker: Attacker,
        adjacentRivers: List<Position>,
    ): List<Position> {
        ...
        val bestRiver =
            adjacentRivers.minByOrNull { riverPos ->
    ...
    fun canBuildBridge(attacker: Attacker): List<Position> {
        ...
        val adjacentRivers =
            attackerPos.getHexNeighbors().filter { pos ->
        ...
        if (attacker.type == AttackerType.OGRE) {
            ...
            val twoTileSpans =
                adjacentRivers.flatMap { firstRiver ->
                    firstRiver
                        .getHexNeighbors()
                        .filter { secondRiver ->
     ...
     */
    private fun isRoughlyInSameDirection(
        start: Position,
        mid: Position,
        end: Position,
    ): Boolean {
        ...
        when (attacker.type) {
            ...
            AttackerType.EVIL_WIZARD, AttackerType.EWHAD -> {
                ...
                if (attacker.position.value
                    ...
                    if (LogConfig.ENABLE_GAME_STATE_LOGGING) {
                        println(
                            "${attacker.type} ${attacker.id} built magical bridge at $bridgePos and moved onto it, level ${attacker.level.value + 1} -> ${attacker.level.value}",
                        )
                    }
                } else {
                    if (LogConfig.ENABLE_GAME_STATE_LOGGING) {
                        println(
                            "${attacker.type} ${attacker.id} built magical bridge at $bridgePos, level ${attacker.level.value + 1} -> ${attacker.level.value}",
                        )
                    }
                }
                return true
