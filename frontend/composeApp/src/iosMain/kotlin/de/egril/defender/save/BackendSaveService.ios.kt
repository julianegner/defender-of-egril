...
actual object BackendSaveService {
    actual suspend fun uploadSavefile(
        ...
        suspendCancellableCoroutine { continuation ->
            val url = NSURL.URLWithString("$backendUrl/api/savefiles")
            if (url == null) {
    ...
    actual suspend fun fetchSavefiles(token: String): List<RemoteSavefileInfo>? =
        suspendCancellableCoroutine { continuation ->
            ...
            NSURLSession.sharedSession
                .dataTaskWithRequest(request) { data, response, _ ->
                    val httpResponse = response as? NSHTTPURLResponse
                    ...
                    if (json == null) {
                        ...
                        continuation.resume(parseRemoteSavefilesJson(json))
                    }
                }.resume()
        }
}
