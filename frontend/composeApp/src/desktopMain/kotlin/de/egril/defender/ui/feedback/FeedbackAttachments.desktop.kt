...
actual suspend fun captureScreenshot(): FeedbackAttachment? = withContext(Dispatchers.IO) {
    try {
        val robot = Robot()
...
@OptIn(ExperimentalEncodingApi::class)
actual suspend fun pickFeedbackFiles(): List<FeedbackAttachment> = withContext(Dispatchers.IO) {
    try {
