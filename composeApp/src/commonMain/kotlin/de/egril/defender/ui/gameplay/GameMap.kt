...
fun GridCell(
    ...
    // Determine if we should use blended tile rendering
    val useTileImages = de.egril.defender.ui.settings.AppSettings.useTileImages.value
    val shouldUseBlendedTiles = useTileImages && tilePainter != null
    ...
    // Pre-compute neighbor tile types (since getTilePainter is @Composable)
    val neighborTileTypes = remember(position, gameState.defenders.size, gameState.level) {
        if (!shouldUseBlendedTiles) {
            emptyMap()
        } else {
...
/**
 * Content displayed inside a grid cell (separated for reuse between BaseGridCell and BlendedTileCell)
 */
@Composable
