...
object MapGenerator {
    ...
    /**
     * Create "The Winding Path" map - 80x80 with winding path from upper left to lower right
     * Features:
     * - 3 spawn points in upper left corner
     * - Target in lower right corner
     * - Winding path 2-3 tiles wide connecting them
     * - Most of map is non-playable
     * - Some build areas adjacent to path
     */
    fun createWindingPathMap(
        id: String = "map_winding_path",
        name: String = "The Winding Path",
        width: Int = 80,
        height: Int = 80
    ): EditorMap {
        val tiles = mutableMapOf<String, TileType>()
        
        // Set spawn points in upper left (3 spawn points)
        tiles["2,2"] = TileType.SPAWN_POINT
        tiles["4,4"] = TileType.SPAWN_POINT
        tiles["2,6"] = TileType.SPAWN_POINT
        
        // Set target in lower right
        tiles["${width - 3},${height - 3}"] = TileType.TARGET
        
        // Create winding path from upper left to lower right
        val windingPath = generateWindingPath(
            start = Position(3, 4),  // Central starting point
            end = Position(width - 3, height - 3),
            width = width,
            height = height
        )
        
        // Add path tiles
        windingPath.forEach { pos ->
            val key = "${pos.x},${pos.y}"
            if (!tiles.containsKey(key)) {
                tiles[key] = TileType.PATH
            }
        ...
        // Fill remaining tiles with NO_PLAY
        for (x in 0 until width) {
            for (y in 0 until height) {
                val key = "$x,$y"
                if (!tiles.containsKey(key)) {
    ...
    /**
     * Generate a winding path with 2-3 tile width from start to end
     * Creates a natural S-curve pattern with varying width
     */
    private fun generateWindingPath(
        ...
        // Create waypoints for the winding path
        val waypoints = mutableListOf<Position>()
        waypoints.add(start)
        
        // Create a winding path by adding waypoints
        // Path goes: upper-left -> upper-middle -> center -> lower-middle -> lower-right
        val segments = 8  // Number of segments in the winding path
        
        for (i in 1 until segments) {
            val progress = i.toDouble() / segments
            
            // Calculate base position along the diagonal
            val baseX = (start.x + (end.x - start.x) * progress).toInt()
            val baseY = (start.y + (end.y - start.y) * progress).toInt()
            
            // Add sinusoidal offset to create winding effect
            // Amplitude varies to create more interesting path
            val amplitude = when {
                progress < 0.2 -> 8  // Start gentle
                progress < 0.4 -> 15 // Wind more
                progress < 0.6 -> 12 // Medium winding
                progress < 0.8 -> 18 // Maximum winding
                else -> 10  // Settle down toward end
            }
            
            val offset = (amplitude * kotlin.math.sin(progress * kotlin.math.PI * 4)).toInt()
            
            // Alternate between horizontal and vertical offsets for more natural winding
            val waypoint = if (i % 2 == 0) {
                Position(baseX + offset, baseY)
            } else {
                Position(baseX, baseY + offset)
            }
        ...
        for (i in 0 until waypoints.size - 1) {
            val segment = createPathBetween(waypoints[i], waypoints[i + 1], maxOf(width, height))
            path.addAll(segment)
        }
        
        // Widen the path to 2-3 tiles
        val widenedPath = mutableSetOf<Position>()
        widenedPath.addAll(path)
